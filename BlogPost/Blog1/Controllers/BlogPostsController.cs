...
public class BlogPostsController : ControllerBase
{
    ...
    {
        return _context.BlogPost.OrderByDescending(p =>
        p.PostId);
    }
    // GET: api/BlogPosts/5
    [HttpGet("{id}")]
...
int id)
    ...
    // PUT: api/BlogPosts/5
    [HttpPut("{id}")]
    public async Task<IActionResult> PutBlogPost([FromRoute]
int id, [FromBody] BlogPost blogPost)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ModelState);
...
BlogPost blogPost)
    {
        ...
        _repo.Add(blogPost);
        var save = await _repo.SaveAsync(blogPost);
        return CreatedAtAction("GetBlogPost", new
        {
            id =
        blogPost.PostId
        }, blogPost);
